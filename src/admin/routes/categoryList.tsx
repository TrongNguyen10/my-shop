...
const CategoryList: React.FC = () => {
    ...
    const searchCategory = (value: string) => {
        var filteredData = value.trim()
            ? allCategoriesData?.filter((category: any) =>
                  category?.name
                      .toLowerCase()
    ...
    useEffect(() => {
        if (categoriesQuery.data && productsQuery.data) {
            categoriesQuery.data?.forEach((category: any) => {
                return (category["data"] = productsQuery.data?.filter(
    ...
    const handleUpdateClick = (e: any) => {
        ...
        if (
            confirm(
                `Cập nhật danh mục mới cho tất cả sản phẩm loại "${oldCategory}" ?`
            )
        ) {
    ...
    async function updateCategoryNames() {
        var updatedProducts = productsQuery.data.filter(
            (product: any) => product.category == oldCategory.trim()
        );

        try {
            for (const product of updatedProducts) {
                await axios.patch(productsAPI + `/${product.id}`, {
                    category: newCategory,
                });
            }
            alert(
                "Cập nhật danh mục mới cho sản phẩm thành công. Vui lòng kiểm tra danh sách sản phẩm !"
            );
    ...
    return (
        <div style={categoryListStyle}>
            ...
            >
                ...
                >
                    <IconButton
                        onClick={() => searchCategory(searchValue)}
                        sx={{ transform: "translateX(2.5rem)", color: "#fff" }}
                    >
                        <Search />
                    </IconButton>
                    <input
