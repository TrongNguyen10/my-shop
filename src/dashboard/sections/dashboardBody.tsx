import { Card, CardContent, CardMedia, Grid2, IconButton, Rating, Typography } from "@mui/material"
    ...
    return (
        <>
            ...
            <div style={{ backgroundColor: '#f6f6f6', padding: '5rem 0' }}>
                <Grid2 sx={{ maxWidth: 1200, margin: 'auto' }} container spacing={2}>
                    <Grid2 size={12}>
                    ...
                    <Grid2 size={12}>
                        ...
                        {props.searchedProducts.length ? <Typography sx={{ mt: 1.5 }}>Kết quả tìm kiếm cho <span style={{ color: 'red' }}>{props.searchValue}</span></Typography> : null}
                    </Grid2>
                    ...
                    {props.query.error || props.query.isLoading || props.displayedProducts.length ? props.displayedProducts?.map((product: any) => (
                        <Grid2 size={{ xs: 12, sm: 6, md: 3 }}>
                            <Card className="cardProduct">
                                <Link onClick={() => props.setClickedProduct(product)} to={`/product/${product.title}`} target="_self">
                                    <CardMedia style={{ backgroundImage: `url(${product.image})` }} sx={props.cardStyle} />
                                </Link>
                                <CardContent>
                                    <Typography sx={props.productTitleStyle} variant="h6">{product.title}</Typography>
                                    <Rating name="read-only" value={product.rating.rate} readOnly size='small' />
                                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                                        <Typography variant="subtitle1">${product.price}</Typography>
                                        <IconButton onClick={() => props.handleAddToCart(product)} color="primary">
                                            <AddShoppingCart />
                                        </IconButton>
                                    </div>
                                </CardContent>
                    ...
                    )) : <h3 style={{ margin: '2.5rem auto 0' }}>Không tìm thấy sản phẩm nào !</h3>}
                </Grid2>
            </div>
            <Contact />
        </>
